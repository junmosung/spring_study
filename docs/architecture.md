...
Spring MVC 학습용 프로젝트의 환경 구성과 전반적인 흐름을 정리한 문서.
...
```
spring01/
├── build.gradle.kts          # 의존성, 플러그인, 컴파일러 옵션
├── settings.gradle.kts
├── Dockerfile                # 멀티 스테이지 빌드 (build → run)
...
└── src/
    ...
    │   │   └── Spring01Application.kt  # @SpringBootApplication 진입점
    │   └── resources/
    │       ├── application.yaml        # 환경변수 참조 기반 설정
    │       └── db/migration/           # Flyway SQL 마이그레이션 위치 (V1__init.sql 등)
    └── test/
        └── kotlin/me/victor/spring01/
